...
module.exports = Joi.object({
  ...
  district: Joi.string().error(Error("Invalid district")),
  address: Joi.string().error(Error("Invalid address")),
  type: Joi.string()
    .pattern(new RegExp(/^(rent|sale)$/))
    .error(Error("Invalid type sale or rent")),
  title: Joi.string().error(Error("Invalid title")),
  description: Joi.string().error(Error("Invalid description")),
  price: Joi.number().error(Error("Invalid price")),
